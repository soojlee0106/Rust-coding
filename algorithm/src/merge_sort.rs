fn merge_sort(array: &[usize]) -> Vec<usize> {
    if array.len() < 2 {
        //already sorted
        return array.to_vec();
    };
    ...
    let lower_array = merge_sort(&array[0..mid_index]);
    let higher_array = merge_sort(&array[mid_index..array.len()]);

    let mut merged_array: Vec<usize> = Vec::with_capacity(array.len());
    let mut low_index = 0;
    ...
    merged_array.extend((&lower_array[low_index..lower_array.len()]).iter());
    merged_array.extend((&higher_array[high_index..higher_array.len()]).iter());
    merged_array
}
